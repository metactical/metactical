...
def mark_source_lines_received(pr_name):
	"""
	Called after a PR is submitted. Flips received_flag on the source rows
	that fed this PR, and marks boxes received if all their items are done.
	...
	for item in pr.items:
		...
		if source_doctype and source_detail:
			child_table = "items"
			frappe.db.set_value(
				source_doctype + " Item" if source_doctype != "Inbound Shipment" else "Inbound Shipment Item",
				source_detail,
				"received_flag",
				1,
			)
	...
	for item in pr.items:
		if item.get("neb_source_doctype") == "Inbound Shipment" and item.get("neb_box_no"):
			ins_name   = item.get("neb_source_name")
			box_no     = item.get("neb_box_no")
			ins        = frappe.get_doc("Inbound Shipment", ins_name)
			box_items  = [l for l in ins.items if l.box_no == box_no]
