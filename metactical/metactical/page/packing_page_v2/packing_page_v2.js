...
frappe.pages['packing-page-v2'].on_page_load = function(wrapper) {
	// Check if user has permisssion to add
	frappe.call({
		method: "metactical.metactical.page.packing_page_v2.packing_page_v2.check_to_add_permission",
		freeze: true,
	...
	frappe.call({
		method: "metactical.metactical.page.packing_page_v2.packing_page_v2.get_default_warehouse",
		freeze: true,
		callback: function(ret){
...
class PackingPage {
	...
	make_page_form(wrapper) {
		...
		this.wrapper.html(
			frappe.render_template("packing_page_v2", {
			...
			df: {
				...
				change: function(){
					$('.picklist-tote-wrapper').on('keypress', function(event){
						...
						{
							frappe.call({
								method: "metactical.metactical.page.packing_page_v2.packing_page_v2.get_delivery_from_tote",
								args: {
...
function populate_pending_items() {
	...
	let items_template = "";
	if (items.length > 0) {
		items_template = frappe.render_template("pending_items_v2", {
			items: items,
		});
	}
...
function populate_packed_items() {
	...
	if(items.length > 0){
		items_template = frappe.render_template("packing_items_v2", {
			items: items,
		});
	} 
...
metactical.packing_page.fetch_dn_items = (from_refresh = false) => {
	...
	if (!delivery_note) {
		let template = frappe.render_template("packing_page_v2", {
			no_data_feedback: "Select Delivery Note",
			delivery_note: 0,
		});
	...
	pending_items.forEach(function(cur_item){
		if (cur_item.item_barcode.indexOf(barcode) != -1) {
			...
			if(cur_item.net_weight == 0){
				...
				function(values){
					frappe.call({
						method: "metactical.metactical.page.packing_page_v2.packing_page_v2.set_item_weight",
						args: {
