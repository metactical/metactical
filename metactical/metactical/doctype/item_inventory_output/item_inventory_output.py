...
# ste refers to stock ledger entry
# def on_sle_update(doc, method):
# 	try:
# 		lead_sources = frappe.get_all('Lead Source')

# 		website_deduct_qty = frappe.get_all('Website Deduct Qty', filters={'parent': doc.item_code}, fields=["lead_source", "qty"])
# 		website_deduct_qty_dict = {x.lead_source: x.qty for x in website_deduct_qty}

# 		all_bins = frappe.get_all('Bin', filters={'item_code': doc.item_code}, fields=["warehouse", "actual_qty", "reserved_qty"])
# 		all_bins = {x.warehouse: x.actual_qty + doc.actual_qty - x.reserved_qty for x in all_bins}

# 		item_inventory_output = frappe.db.exists('Item Inventory Output', {'name': doc.item_code})

# 		if not item_inventory_output:
# 			item_inventory_output = frappe.new_doc('Item Inventory Output')
# 			item_inventory_output.item_code = doc.item_code
# 		else:
# 			item_inventory_output = frappe.get_doc('Item Inventory Output', item_inventory_output)
# 			item_inventory_output.item_inventory_output_list = []

# 		for lead_source in lead_sources:
# 			allowed_warehouses = frappe.get_all('SB Warehouse Inventory Sync', filters={'parent': lead_source.name}, fields=["warehouse"])
# 			allowed_warehouses = [x.warehouse for x in allowed_warehouses]

# 			qty_to_deduct = website_deduct_qty_dict.get(lead_source.name, 0)
# 			qty_to_send_to_sb = 0
# 			total_available_qty = 0

# 			for warehouse in allowed_warehouses:
# 				total_available_qty += all_bins.get(warehouse, 0)

# 			qty_to_send_to_sb = total_available_qty - qty_to_deduct
