...
# 	"""
# 	Extract unique values from the Excel file grouped by Price List
# 	Only includes items that have a price for each specific price list
# 	Makes separate API calls for each enabled price list
	
# 	Extraction is done ONLY from Variants sheet for items with prices
# 	Specification labels are extracted from the third sheet (Website Specifications)
...
# 		# Get headers from Variants sheet
# 		variants_headers = []
# 		for idx, row in enumerate(variants_sheet.iter_rows(values_only=True)):
# 			if idx == 0:
# 				variants_headers = list(row)
# 				break
		
# 		# Find the Cost column index (price lists start after this)
# 		cost_column_index = -1
# 		if 'Cost' in variants_headers:
# 			cost_column_index = variants_headers.index('Cost')
# 		else:
# 			frappe.throw("Cost column not found in Variants sheet")
		
# 		# Get all price list columns (after Cost)
# 		price_list_columns = {}
# 		for i, header in enumerate(variants_headers):
# 			if i > cost_column_index and header and header.strip():
# 				# This is a price list column
...
# 		# Get enabled validation configs
...
# 		# Create a mapping of price list to validation config
# 		price_list_to_config = {}
# 		for config in validation_configs:
# 			price_list_to_config[config.price_list] = config
		
# 		# Get column indices for Variants sheet (no longer using Attribute column)
# 		variants_category_idx = variants_headers.index('Item Group') if 'Item Group' in variants_headers else -1
# 		variants_brand_idx = variants_headers.index('Brand') if 'Brand' in variants_headers else -1
# 		variants_availability_idx = variants_headers.index('VariantAvailabilityRule') if 'VariantAvailabilityRule' in variants_headers else -1
# 		variants_item_code_idx = variants_headers.index('Item Code') if 'Item Code' in variants_headers else 0

		
# 		# Extract specification labels from third sheet
# 		specs_by_item = {}  # Map of item_code -> set of spec labels
		
# 		if len(wb.sheetnames) >= 3:
# 			specs_sheet = wb[wb.sheetnames[2]]
			
# 			# Get spec sheet headers (first row)
# 			spec_headers = []
...
# 			if spec_headers and len(spec_headers) > 1:
# 				# First column is Item Code, rest are spec labels
# 				spec_labels = []
...
# 						# If the cell has a value (not None, not empty), include this spec label
# 						if len(row) > col_idx and row[col_idx] is not None:
# 							description = str(row[col_idx]).strip()
# 							if description:  # Only add if description is not empty
...
# 					if item_specs:
# 						specs_by_item[item_code] = item_specs
				
# 				frappe.logger().info(f"Extracted specs for {len(specs_by_item)} items from third sheet")
# 			else:
# 				frappe.logger().warning("Third sheet found but has invalid structure")
# 		else:
# 			frappe.logger().info("No third sheet found for specifications")
		
# 		# Store results grouped by price list
# 		results = {}
		
# 		# Process each price list
# 		for price_list_name, price_column_idx in price_list_columns.items():
# 			# Check if this price list has a validation config
# 			if price_list_name not in price_list_to_config:
# 				frappe.msgprint(f"Price list '{price_list_name}' found in Excel but no validation config exists. Skipping.")
# 				continue
			
# 			config = price_list_to_config[price_list_name]
			
# 			# Initialize sets for this price list
# 			specs_external_ids = set()
# 			categories_external_ids = set()
# 			brands_external_ids = set()
# 			availability_rules_aliases = set()
			
# 			# Collect from Variants sheet - ONLY for items with price in this price list
# 			for idx, row in enumerate(variants_sheet.iter_rows(values_only=True)):
# 				if idx == 0:  # Skip header
# 					continue
				
# 				# Check if this item has a price for this price list
# 				has_price = False
...
# 				# Only include this item's data if it has a price
# 				if has_price:
# 					# Get item code for this row
# 					item_code = None
# 					if len(row) > variants_item_code_idx and row[variants_item_code_idx]:
# 						item_code = str(row[variants_item_code_idx]).strip()
					
# 					# Extract Specs from third sheet (if item has specs)
# 					if item_code and item_code in specs_by_item:
# 						specs_external_ids.update(specs_by_item[item_code])
					
# 					# Extract Categories
# 					if variants_category_idx != -1 and len(row) > variants_category_idx and row[variants_category_idx]:
# 						val = str(row[variants_category_idx]).strip()
# 						if val:
# 							categories_external_ids.add(val)
					
# 					# Extract Brands
# 					if variants_brand_idx != -1 and len(row) > variants_brand_idx and row[variants_brand_idx]:
# 						val = str(row[variants_brand_idx]).strip()
# 						if val:
# 							brands_external_ids.add(val)
					
# 					# Extract Availability Rules
# 					if variants_availability_idx != -1 and len(row) > variants_availability_idx and row[variants_availability_idx]:
# 						val = str(row[variants_availability_idx]).strip()
# 						if val:
# 							availability_rules_aliases.add(val)
			
# 			# Create payload for this price list
# 			payload = {
...
# 			# Make API call for this price list
# 			try:
# 				headers = {
