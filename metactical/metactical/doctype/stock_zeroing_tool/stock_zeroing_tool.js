...
frappe.provide("metactical");

frappe.ui.form.on('Stock Zeroing Tool', {
	onload: function (frm) {
	...
	get_items: function (frm) {
		...
		frappe.prompt(
			...
			function (data) {
				frappe.call({
					method: "metactical.metactical.doctype.stock_zeroing_tool.stock_zeroing_tool.get_items",
					args: {
	...
	set_valuation_rate_and_qty: function (frm, cdt, cdn) {
		...
		if (d.item_code && d.warehouse) {
			frappe.call({
				method: "metactical.metactical.doctype.stock_zeroing_tool.stock_zeroing_tool.get_stock_balance_for",
				args: {
	...
	set_expense_account: function (frm) {
		if (frm.doc.company && erpnext.is_perpetual_inventory_enabled(frm.doc.company)) {
			return frm.call({
				method: "metactical.metactical.doctype.stock_zeroing_tool.stock_zeroing_tool.get_difference_account",
				args: {
...
frappe.ui.form.on("Stock Zeroing Item", {
...
metactical.StockZeroingTool = class StockZeroingTool extends erpnext.stock.StockController {
	setup() {
		var me = this;
...
cur_frm.cscript = new metactical.StockZeroingTool({ frm: cur_frm });
