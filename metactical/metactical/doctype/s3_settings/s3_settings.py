...
class S3Settings(Document):
	def get_client(self, ignore_disabled=False):
		...
		"""
		if self.nat_disabled and not ignore_disabled:
			frappe.throw("S3 Uploader is disabled. Enable it in S3 Settings.")

		if not (self.nat_bucket_name and self.nat_region and self.nat_aws_access_key_id):
			frappe.throw("S3 Settings are incomplete. Set bucket, region and credentials.")

		import boto3

		secret = self.get_password("nat_aws_secret_access_key", raise_exception=False)
		if not secret:
			frappe.throw("S3 Settings are missing the AWS secret access key.")

		return boto3.client(
			"s3",
			region_name=self.nat_region,
			aws_access_key_id=self.nat_aws_access_key_id,
	...
	def public_url_base(self):
		...
		if not (self.nat_bucket_name and self.nat_region):
			return ""
		return f"https://{self.nat_bucket_name}.s3.{self.nat_region}.amazonaws.com"

	def get_public_config(self):
		"""Return only non-secret config for the frontend (never the secret key)."""
		return {
			"disabled": bool(self.nat_disabled),
			"bucket_name": self.nat_bucket_name,
			"region": self.nat_region,
