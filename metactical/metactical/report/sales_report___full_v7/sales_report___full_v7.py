...
def execute(filters=None):
	...
	for i in master:
		...
		row["material_request"], row['mr_status'], row['mr_total_qty'] = get_open_material_request(i.get("item_code"))
		...
		if row.get("total_actual_qty", 0) <= row.get("ais_poreorderlevel", 0):
...
def get_pr_draft( item, po_name):
	output = ""
	data = frappe.db.sql("""select pr.name, ri.qty from `tabPurchase Receipt` pr inner join 
			`tabPurchase Receipt Item` ri on  pr.name = ri.parent		
			where pr.docstatus=0 and ri.item_code = %s
		 	and pr.purchase_order = %s""",(item, po_name))
	for d in data:
		output += d[0]+" ("+str(d[1])+"), "	
	return data
...
def get_pr_qty( item, po_name):
	qty = 0
	data = frappe.db.sql("""select ri.qty from `tabPurchase Receipt` pr inner join 
			`tabPurchase Receipt Item` ri on  pr.name = ri.parent		
			where pr.docstatus=0 and ri.item_code = %s
		 	and pr.purchase_order = %s""",(item, po_name))
	for d in data:
