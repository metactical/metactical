...
import requests
from metactical.api.sales_report import get_us_sales_data
...
def execute(filters=None):
	...
	master = get_master(conditions, params)
	
	#Get US data
	us_data = {}
	us_data = get_us_sales_data(params)
...
def get_mthsck(item_code, warehouse, today):
	...
	data =  frappe.db.sql("""
		...
		and `tabSales Invoice`.company = 'International Camouflage Ltd'
...
def get_avgorderqty(last_year, item_code):
	data =  frappe.db.sql("""
		...
		where `tabPurchase Receipt`.docstatus =1 
			...
			and `tabPurchase Receipt`.company = 'International Camouflage Ltd'
	""", (item_code, str(last_year)+"-01-01",str(last_year)+"-12-30"), as_dict=1)
...
def get_endinvt(item_code, warehouse, today):
	...
	data = frappe.db.sql("""
		...
		where item_code =%s and posting_date between %s and %s 
			and company = 'International Camouflage Ltd'
	""",(item_code, fromdate, enddate), as_dict=1)
	total = 0
	if data[0].total:
		total = data[0].total
...
def get_master(conditions="", filters={}):
	if "supplier" in filters and filters.get("supplier"):
		filters["supplier"] = tuple(frappe.parse_json(filters.get("supplier")))
...
def get_conditions(filters):
    ...
    return where_clause + limit_clause, params
...
def get_open_po_qty(item,supplier, warehouse=None):
	...
	prs = frappe.db.sql("""select 
							...
							inner join 
								`tabPurchase Receipt` p on p.name = ri.parent
							where
	...
	if pos: total_po_qty = pos[0].qty if pos[0].qty else 0
	if prs: total_pr_qty = prs[0].qty if prs[0].qty else 0
	 
	total_qty = total_po_qty - total_pr_qty
	return total_qty
