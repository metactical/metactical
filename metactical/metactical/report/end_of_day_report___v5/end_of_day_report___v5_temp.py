	...
	for source in sources:
		...
		if matches:
			...
			if doctype == "tabSales Order":
				sql = get_website_orders_sql(source.name, date)

			elif doctype == "tabSales Invoice":
				sql = get_stores_sql(source.name, date)
...
def get_website_orders_sql(source, date):
	return """SELECT 
				COALESCE(SUM(net_total), 0) AS total_with_tax
			FROM
				`tabSales Order`
			JOIN
				`tabPayment Entry Reference` ON `tabPayment Entry Reference`.reference_name = `tabSales Order`.name
			JOIN
				`tabPayment Entry` ON `tabPayment Entry`.name = `tabPayment Entry Reference`.parent
			WHERE
				source = %(source)s AND transaction_date = %(date)s AND docstatus = 1
			AND `tabPayment Entry`.paid_amount >= `tabSales Order`.grand_total
		""".format(source=source, date=date)
   
def get_stores_sql(source, date):
	return """SELECT 
						COALESCE(SUM(net_total), 0) AS total_without_tax
					FROM
						`tabSales Invoice`
					WHERE
						source = %(source)s AND neb_payment_completed_at = %(date)s
						AND docstatus = 1""".format(source=source, date=date)
