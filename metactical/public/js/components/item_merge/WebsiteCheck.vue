<template>
  <!-- Storebuilder maps a product's variants through its External ID, which must equal the ERP template
       code. Per website row: External ID vs template code and variant counts, asked live from each site. -->
  <div class="space-y-2">
    ...
    <div v-if="result && result.rows.length" class="im-table-wrap">
      <table class="im-table">
        <thead>
          <tr><th>Price list</th><th>Website</th><th>External ID</th><th class="r">Variants site / ERP</th><th>Status</th></tr>
        </thead>
        <tbody>
    ...
    <p v-if="result && result.rows.length" class="text-xs text-muted mb-0">
      Asked live from each website. After a merge or a template rename, fix the External ID in Storebuilder admin, then Recheck.
    </p>
  </div>
</template>
