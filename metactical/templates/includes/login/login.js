...
login.bind_events = function() {
	...
	$("#login_email").on("focusout", function(){
		var user = frappe.utils.xss_sanitise(($("#login_email").val() || "").trim());
		if(user != ''){
			frappe.call({
				"method": "metactical.custom_scripts.employee_checkin.employee_checkin.check_user",
				"args": {"user": user},
				"callback": function(ret){
					console.log(ret)
					if(ret.message){
						window.userstatus = 'Employee'
						if(ret.message['is_user'] == 1){
							if(ret.message['last_status'] == "IN"){
								$('.btn-checkin').html('Checkout');
							}
							else if(ret.message['last_status'] == 'OUT'){
								$('.btn-checkin').html('Checkin');
							}
						}
						else{
							window.userstatus = 'Not employee'
						}
					}
				}
			});
	...
	$(".btn-checkin").on("click", function(event) {
		if(window.userstatus && window.userstatus == 'Employee'){
			event.preventDefault();
			var args = {};
			args.cmd = "login";
			args.usr = frappe.utils.xss_sanitise(($("#login_email").val() || "").trim());
			...
			frappe.call({
				...
				args: {'user': args.usr, 'password': args.pwd},
				callback: function(ret){
					console.log(ret);
					login.set_indicator('{{ _("Success") }}', 'green');
					if(ret.message){
						frappe.msgprint("You've successfully checked in/out")
						$('.btn-checkin').html(ret.message['next_log'])
					}
				},
				freeze: true
			});
